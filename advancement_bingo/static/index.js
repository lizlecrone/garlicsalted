...
function Footer({enabled, onClick, number, children}) {
    return(
        <div
            style={{
                width: '100%',
...
function Customize () {
    ...
    function handleDownload() {
        ...
        const zipPath = `/generate/hunt/custom?selected=${selectedJson}`
        window.open(zipPath)
    }
...
function Randomize () {
    ...
    function handleRandomDownload() {
        const zipPath = isSeries ? '/generate/series/random' : '/generate/hunt/random'
        window.open(zipPath)
    }
            ...
            >
                <div style={{margin: '32px 16px 0 16px'}}>
                    <input
                        ...
                        type="checkbox"
                        checked={isSeries}
                        onClick={() => {setIsSeries(i => !i)}}
                        style={{cursor: 'pointer'}}
                    />
                    <label for="series" style={{
                        color: 'white',
                        fontFamily: 'MinecraftiaRegular',
                        padding: '24px',
                    }}>
                        Lockout mode (only 1 advancement at a time)
                    </label>
                </div>
            </Footer>
        </div>
